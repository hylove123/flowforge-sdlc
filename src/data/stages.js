...
export const STAGE_NAMES = STAGE_DEFINITIONS.map(s => s.name)
...
export function buildDefaultStageConfigs() {
  const configs = {}
...
export const DEFAULT_STAGE_AGENTS = {
...
export function buildFlowConfig(customStages = null) {
  ...
  if (typeof customStages[0] === 'object' && customStages[0].stage) {
    return customStages.map((item, idx) => {
  ...
  return customStages.map((stageId, idx) => {
...
export function getProjectFlowConfig(project = null) {
...
export function getProjectStages(project = null) {
